        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($students as $student)
                        <tr wire:key="student-{{ $student->id }}">
                            ...
                            <td class="px-6 py-4 text-sm text-right space-x-2">
                                @can('update', $student)
                                    <a href="{{ route('students.students.edit', $student) }}" wire:navigate class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Edit') }}</a>
                                @endcan
                                @can('delete', $student)
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ __('No students yet.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
